...
void MAIN_MENU_INPUT_CTRL(char ov_menu_input, char *ov_uid) {
	if ((ov_menu_input != 'e' && ov_menu_input != 'c')
		&& (ov_menu_input != 'a' && ov_menu_input != 'm')) BB_ERROR(ov_uid);
}
...
bool CALCULATOR_INPUT_CTRL(unsigned int c_input, bool CHECK, char *ov_uid) {
...
bool ATM_INPUT_CTRL(float atm_input, bool CHECK, char *ov_uid) {
